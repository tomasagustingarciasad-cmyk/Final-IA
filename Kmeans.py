...
# Config por defecto
# ----------------------------
CSV_DEFAULT_NAME = "features_imagenes.csv"
...
def plot_2d_and_3d(df, labels, kmeans, scaler, mayoritaria):
    ...
    scat(axes[1], df['hu1_log'], df['aspect_ratio'], 'hu1_log vs aspect_ratio', 'hu1_log', 'aspect_ratio')
    scat(axes[2], df['hu2_log'], df['aspect_ratio'], 'hu2_log vs aspect_ratio', 'hu2_log', 'aspect_ratio')

    cent_scaled = kmeans.cluster_centers_
    cent_orig = scaler.inverse_transform(cent_scaled)  # [hu1_log, hu2_log, aspect_ratio]
    ...
    p3 = ax3.scatter(df['hu1_log'], df['hu2_log'], df['aspect_ratio'],
                     c=labels, cmap=cmap, alpha=0.85, s=20)
    ax3.set_xlabel('hu1_log'); ax3.set_ylabel('hu2_log'); ax3.set_zlabel('aspect_ratio')
    ax3.set_title('Espacio 3D: hu1_log, hu2_log, aspect_ratio')
    # centroides en 3D
...
def main():
    ...
    req_cols = {'hu1','hu2','aspect_ratio'}
    faltan = req_cols - set(df.columns)
    if faltan:
        raise ValueError(f"Faltan columnas en {csv_path.name}: {faltan}")

    df = df.replace([np.inf, -np.inf], np.nan).dropna(subset=['hu1','hu2','aspect_ratio']).copy()
    if df.empty:
    ...
    X = df[['hu1_log','hu2_log','aspect_ratio']].values
